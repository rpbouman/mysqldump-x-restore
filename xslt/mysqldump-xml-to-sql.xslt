    ...
    <xsl:choose>
        <xsl:when test="contains($value, $apos)">
            <xsl:call-template name="replace-string">
                ...
                <xsl:with-param name="with" select='"&apos;&apos;"'/> <!-- escape with '' which is the MySQL standard -->
            </xsl:call-template>
        </xsl:when>
        <xsl:otherwise><xsl:value-of select="$value"/></xsl:otherwise>
    </xsl:choose>
</xsl:template>
<!-- Reference: https://stackoverflow.com/questions/7711654/xslt-replace-single-quotes-by -->
